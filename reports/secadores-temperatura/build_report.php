...
$buildAdjustmentCue = static function ($value, array $rule): ?array {
  if ($value === null || $value === '' || !is_numeric($value)) {
    return null;
  }
  ...
  if ($mode === 'minimo') {
    if ($greenMin !== null && $number < $greenMin) {
      $delta = $greenMin - $number;
      return [
        'icon' => 'up',
        'label' => 'Subir ' . n($delta, 1) . ' °C',
      ];
    }

    if ($greenMin !== null && $number > $greenMin) {
      $delta = $number - $greenMin;
      return [
        'icon' => 'down',
        'label' => 'Bajar ' . n($delta, 1) . ' °C',
      ];
    }
...
$buildFieldRangeLabel = static function (array $rule, ?string $override = null): string {
  if ($override !== null && trim($override) !== '') {
    return trim($override);
  }

  $mode = (string)($rule['modo'] ?? 'rango');
  $verdeMin = isset($rule['verde_min']) && is_numeric($rule['verde_min']) ? (float)$rule['verde_min'] : null;
  $verdeMax = isset($rule['verde_max']) && is_numeric($rule['verde_max']) ? (float)$rule['verde_max'] : null;
  $amarilloMin = isset($rule['amarillo_min']) && is_numeric($rule['amarillo_min']) ? (float)$rule['amarillo_min'] : null;
  $amarilloMax = isset($rule['amarillo_max']) && is_numeric($rule['amarillo_max']) ? (float)$rule['amarillo_max'] : null;

  $fmt = static function (?float $value): string {
    return $value === null ? '-' : n($value, 1);
  };

  if ($mode === 'minimo') {
    return 'Verde >= ' . $fmt($verdeMin) . ' | Amarillo >= ' . $fmt($amarilloMin);
  }

  if ($mode === 'maximo') {
    return 'Verde <= ' . $fmt($verdeMax) . ' | Amarillo <= ' . $fmt($amarilloMax);
  }

  return 'Verde ' . $fmt($verdeMin) . ' a ' . $fmt($verdeMax)
    . ' | Amarillo ' . $fmt($amarilloMin) . ' a ' . $fmt($amarilloMax);
};
...
if (!$serverConfigured) {
  $warnings[] = 'Configura la conexión SQL Server en reports/secadores-temperatura/config.php para consultar AVEVA_TAGS.TREND001.';
} else {
  try {
...
foreach ($rows as $row) {
  ...
  $actualTimestamp = $normalizeTimestamp($row['__timestamp'] ?? null);
  if ($actualTimestamp === null) {
    continue;
  }

  $row['__bucket_timestamp'] = $bucketTimestamp;
  $row['__timestamp_distance'] = abs($actualTimestamp->getTimestamp() - $bucketTimestamp->getTimestamp());
  ...
  $currentDistance = (int)($bucketRows[$bucketKey]['__timestamp_distance'] ?? PHP_INT_MAX);
  $candidateDistance = (int)$row['__timestamp_distance'];
  ...
  if ($candidateDistance === $currentDistance) {
    $existingTimestamp = $normalizeTimestamp($bucketRows[$bucketKey]['__timestamp'] ?? null);
    if ($existingTimestamp === null || $actualTimestamp < $existingTimestamp) {
      $bucketRows[$bucketKey] = $row;
    }
  }
}
