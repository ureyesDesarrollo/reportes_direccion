...
<head>
  ...
  <style>
    ...
    .secadores-adjustment.up {
      color: #2563eb;
    }

    .secadores-adjustment.down {
      color: #dc2626;
    }
...
<body>
  <div class="dashboard">
    ...
    <div class="secadores-note">
      ...
      Edita <strong>reports/secadores-temperatura/config.php</strong> para ajustar por variable los límites de color con <code>modo</code> (<code>rango</code>, <code>minimo</code>, <code>maximo</code>) y sus valores <code>verde_*</code> / <code>amarillo_*</code>.<br>
  ...
  <script>
    (() => {
      ...
      const storageKey = 'secadores.activeTunnel';

      const getRequestedTunnel = () => {
        const params = new URLSearchParams(window.location.search);
        const urlTunnel = (params.get('tunel') || '').trim();
        const storedTunnel = (window.localStorage.getItem(storageKey) || '').trim();
        const bootstrapTunnel =
          (window.secadoresBootstrap && window.secadoresBootstrap.tunelSeleccionado) ||
          '';
        const firstTunnel = Object.keys((window.secadoresBootstrap && window.secadoresBootstrap.tuneles) || {})[0] || 'tunel_2';

        return urlTunnel || storedTunnel || bootstrapTunnel || firstTunnel;
      };

      const syncTunnelState = () => {
        if (!state.activeTunnel) {
          return;
        }

        try {
          window.localStorage.setItem(storageKey, state.activeTunnel);
        } catch (error) {
        }

        const url = new URL(window.location.href);
        if (url.searchParams.get('tunel') !== state.activeTunnel) {
          url.searchParams.set('tunel', state.activeTunnel);
          window.history.replaceState({}, '', url.toString());
        }
      };

      state.activeTunnel = getRequestedTunnel();
      ...
      const renderTable = () => {
        ...
        if (tableHead) {
          ...
          (tunnel.campos || []).forEach((field) => {
            const rangeLabel = field.rangeLabel
              ? '<small style="display:block; margin-top:4px; font-weight:600; color:#64748b; line-height:1.3;">' + field.rangeLabel + '</small>'
              : '';
            headerCells.push('<th>' + field.label + rangeLabel + '</th>');
          });
          headerCells.push('</tr>');
        ...
        tableBody.innerHTML = rows.map((row) => {
          const cells = row.cells.map((cell) => {
            ...
            const adjustment = row.isLatest && cell.adjustmentCue
              ? '<span class="secadores-adjustment ' + (cell.adjustmentCue.icon || '') + '">'
                + ((cell.adjustmentCue.icon === 'up') ? '↑ ' : ((cell.adjustmentCue.icon === 'down') ? '↓ ' : ''))
                + (cell.adjustmentCue.label || '')
                + '</span>'
              : '';
      ...
      const renderAll = () => {
        if (!state.payload.tuneles?.[state.activeTunnel]) {
          state.activeTunnel =
            state.payload.tunelSeleccionado ||
            Object.keys(state.payload.tuneles || {})[0] ||
            state.activeTunnel;
        }
        syncTunnelState();
        renderTabs();
