...
<div class="cards-section">
  <div class="pivot-header">
    <div>
      <h2>
        ...
        Matriz de refacciones generales por semana (<?= htmlspecialchars((string)$anioPivot) ?>)
      </h2>
      <div class="cards-sub">
  ...
  <div class="table-wrapper table-wrapper-pivot">
    <table class="table-pivot-quimicos" id="pivotTable">
      ...
      <?php
      $_detailBaseUrl = '../refaccion-general-detalle/index.php';
      $_pivotRows = [];
...
<script>
  (function() {
    ...
    if (sortByConsumo && sortByCosto && table) {
      ...
      // --- old DOM-traversal functions removed (virtual rendering via renderProductPage) ---

      function _legacy_pagination_removed() {
        const total = 0; // dead code - kept as placeholder to avoid parse errors if referenced
      ...
      function _deadCambiarModo(modo) {
        ...
        // Actualizar columnas sticky Anterior / Actual
        table.querySelectorAll('tr[data-quimico]').forEach(row => {
          const tdAnterior = row.querySelector('td[data-valor-anterior]');
          const tdActual = row.querySelector('td[data-valor-actual]');
          const spanAnterior = tdAnterior ? tdAnterior.querySelector('.valor-anterior') : null;
          const spanActual = tdActual ? tdActual.querySelector('.valor-actual') : null;
          const spanVariacion = tdActual ? tdActual.querySelector('.variacion-texto') : null;

          if (modo === 'costo') {
            const costoAnterior = parseFloat(tdAnterior?.getAttribute('data-valor-costo-anterior') || '0');
            const costoActual = parseFloat(tdActual?.getAttribute('data-valor-costo-actual') || '0');
            const varCosto = parseFloat(tdActual?.getAttribute('data-variacion-costo') || '0');
            const colorSemaforo = varCosto > 10 ? '#ef4444' : (varCosto > 0 ? '#f59e0b' : '#10b981');

            if (spanAnterior) spanAnterior.textContent = '$' + costoAnterior.toLocaleString('es-MX', {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2
            });
            if (spanActual) spanActual.textContent = '$' + costoActual.toLocaleString('es-MX', {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2
            });
            if (spanVariacion) {
              spanVariacion.textContent = (varCosto >= 0 ? '+' : '') + varCosto.toLocaleString('es-MX', {
                minimumFractionDigits: 1,
                maximumFractionDigits: 1
              }) + '%';
              spanVariacion.style.color = colorSemaforo;
              if (tdActual) tdActual.style.borderLeftColor = colorSemaforo;
            }
          } else {
            const valorAnterior = parseFloat(tdAnterior?.getAttribute('data-valor-anterior') || '0');
            const valorActual = parseFloat(tdActual?.getAttribute('data-valor-actual') || '0');
            const varConsumo = parseFloat(tdActual?.getAttribute('data-variacion') || '0');
            const colorSemaforo = varConsumo > 10 ? '#ef4444' : (varConsumo > 0 ? '#f59e0b' : '#10b981');

            if (spanAnterior) spanAnterior.textContent = valorAnterior.toLocaleString('es-MX', {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2
            });
            if (spanActual) spanActual.textContent = valorActual.toLocaleString('es-MX', {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2
            });
            if (spanVariacion) {
              spanVariacion.textContent = (varConsumo >= 0 ? '+' : '') + varConsumo.toLocaleString('es-MX', {
                minimumFractionDigits: 1,
                maximumFractionDigits: 1
              }) + '%';
              spanVariacion.style.color = colorSemaforo;
