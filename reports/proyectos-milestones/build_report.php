  ...
  if ($detailProject === null) {
    $detailStmt = $pdo->prepare("
      SELECT
        ...
        'Sin zona' AS zona,
        'Sin área' AS area,
        'Sin tipo' AS tipo_proyecto,
        'Sin departamento' AS responsable_area,
        'En evaluacion' AS estado_directivo,
        '' AS prioridad_directiva,
        0 AS visible_en_direccion,
        0 AS requiere_reporte_direccion,
        ...
        COUNT(DISTINCT CASE
          WHEN t.completada = 1 AND t.completada_en IS NOT NULL AND DATE(t.completada_en) > t.fecha_fin
          THEN t.tarea_id
      ...
      WHERE m.milestone_id = ?
        AND (? = 0 OR e.empresa_id = ?)
      GROUP BY
        m.milestone_id, e.estrategia_id, m.titulo, m.descripcion, m.estatus,
        m.prioridad, m.responsable_usuario_id, u.nombre_completo
      LIMIT 1
    ");
    $detailStmt->execute([$milestoneSelected, $empresaId, $empresaId]);
    $detailProject = $detailStmt->fetch() ?: null;

    if ($detailProject === null) {
      throw new RuntimeException('No se encontró el milestone solicitado.');
    }

    $detailProject['milestone_estatus'] = (int)($detailProject['milestone_estatus'] ?? 0);
    $detailProject['total_tareas'] = (int)($detailProject['total_tareas'] ?? 0);
    $detailProject['tareas_finalizadas'] = (int)($detailProject['tareas_finalizadas'] ?? 0);
    $detailProject['tareas_vencidas'] = (int)($detailProject['tareas_vencidas'] ?? 0);
    $detailProject['tareas_completadas_tarde'] = (int)($detailProject['tareas_completadas_tarde'] ?? 0);
    $detailProject['avance_real'] = (float)($detailProject['avance_real'] ?? 0);
    $detailProject['destacado_orden'] = $highlightOrders[$milestoneSelected] ?? 0;
    $detailProject['es_destacado'] = (int)$detailProject['destacado_orden'] > 0;
    $detailProject['milestone_status'] = $resolveMilestoneStatus($detailProject['milestone_estatus']);
    $detailProject['health'] = $resolveProjectHealth($detailProject);
    $detailProject['pendientes'] = max(0, $detailProject['total_tareas'] - $detailProject['tareas_finalizadas']);
    $detailProject['prioridad_display'] = trim((string)($detailProject['prioridad'] ?? '')) !== ''
      ? (string)$detailProject['prioridad']
      : 'Sin prioridad';
  }
